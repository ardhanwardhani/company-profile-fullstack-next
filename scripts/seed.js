...
async function seed() {
  ...
  try {
    ...
    await client.query(`
      INSERT INTO blog_posts (title, slug, excerpt, content, category_id, author_id, status, seo_index, published_at)
      SELECT 
        ...
        '<p>This is the beginning of our blogging journey. Stay tuned for more updates!</p>',
        id, 
    ...
    await client.query(`
      INSERT INTO jobs (title, slug, description, responsibilities, requirements, apply_url, department_id, location_id, employment_type, status, seo_index, published_at)
      SELECT 
        'Senior Frontend Developer',
        'senior-frontend-developer',
        '<p>We are looking for a Senior Frontend Developer to join our team.</p>',
        '<ul><li>Build and maintain web applications</li></ul>',
        '<ul><li>5+ years of experience</li></ul>',
        'mailto:careers@company.com',
        (SELECT id FROM departments WHERE name = 'Engineering' LIMIT 1),
        (SELECT id FROM locations WHERE name = 'Jakarta' LIMIT 1),
        'full-time',
        'open',
        true,
        NOW()
