...
async function getCategories(params: SearchParams) {
...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { categories, total, totalPages } = await getCategories(resolvedParams);
    ...
    <div className="p-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <form method="GET" className="flex items-end gap-3">
          ...
          {hasFilters && (
            <Link href="/dashboard/master-data/categories" className="px-4 py-2 text-gray-600 hover:text-gray-900 text-sm">
              Clear
            </Link>
          )}
        </form>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {categories.length > 0 ? (
                ...
                {categories.map((category: any) => (
                  <tr key={category.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-4">
                      <Link href={`/dashboard/master-data/categories/${category.id}`} className="font-medium text-gray-900 hover:text-blue-600 transition-colors">
                        {category.name}
                      </Link>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <span className="text-sm text-gray-600 max-w-xs truncate block">{category.description || '-'}</span>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <CategoryStatusButton categoryId={category.id} categoryName={category.name} currentStatus={category.is_active} />
                        <Link href={`/dashboard/master-data/categories/${category.id}/edit`} className="text-sm text-gray-600 hover:text-blue-600 transition-colors">
                          Edit
                        </Link>
                      </div>
                    </td>
            ...
            {totalPages > 1 && (
              <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {hasPrev && (
                    <Link
                      href={`/dashboard/master-data/categories?${new URLSearchParams({
                        ...resolvedParams,
          ...
          <div className="px-4 py-16 text-center">
            ...
            <p className="text-sm text-gray-400">{hasFilters ? 'Try adjusting your filters' : 'Create your first category to get started'}</p>
          </div>
        )}
      </div>
    </div>
